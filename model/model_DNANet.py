...
class DNANet(nn.Module):
    def __init__(self, num_classes, input_channels, block, num_blocks, nb_filter,deep_supervision=False):
        ...
        self.encoder_x2 = torch.nn.TransformerEncoderLayer(64*64, 4, 64*64, dropout=0.5)
        self.encoder_x4 = torch.nn.TransformerEncoderLayer(16*16, 4, 16*16, dropout=0.5)
    ...
    def forward(self, input):
        ...
        if self.training:
            old_shape = x2_0.shape
            x2_0 = x2_0.view(old_shape[0],old_shape[1],old_shape[2]*old_shape[3])
            old_x2_0 = x2_0
            old_x2_0 = x2_0[ :len(x2_0)//2, :, :]
            x2_0 = x2_0[len(x2_0)//2:, :, :]
            x2_0 = self.encoder_x2(x2_0)
            x2_0 = torch.cat([old_x2_0, x2_0], dim=0)
            x2_0 = x2_0.view(old_shape)
        ...
        if self.training:
            old_shape = x4_0.shape
            x4_0 = x4_0.view(old_shape[0],old_shape[1],old_shape[2]*old_shape[3])
            old_x4_0 = x4_0
            old_x4_0 = x4_0[ :len(x4_0)//2, :, :]
            x4_0 = x4_0[len(x4_0)//2:, :, :]
            x4_0 = self.encoder_x4(x4_0)
            x4_0 = torch.cat([old_x4_0, x4_0], dim=0)
            x4_0 = x4_0.view(old_shape)
